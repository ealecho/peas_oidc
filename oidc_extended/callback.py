...
def custom(code: str, state: str | dict):
    ...
    if frappe.db.exists("User", {"username": username}):
        ...
        # Save new user to database first (without roles) so it has a valid user.name for Employee creation
        user.insert()
        frappe.logger().info(f"New user {username} inserted into database with name: {user.name}")
        
        # Create Employee record if enabled (before assigning roles that may require Employee)
        ...
        # Add default role profile if configured (after Employee creation)
        if oidc_extended_configuration.default_role:
            ...
            if frappe.db.exists("Role Profile", default_role_profile):
                frappe.logger().debug(f"Setting default Role Profile: {default_role_profile}")
                user.reload()
                user.role_profile_name = default_role_profile
                user.save()
                frappe.logger().info(f"Set role profile '{default_role_profile}' for user {username}")
            else:
    ...
    # Create Employee record for existing users if enabled and doesn't exist
    if oidc_extended_configuration.auto_create_employee:
        if not frappe.db.exists("Employee", {"user_id": user.name}):
            frappe.logger().info(f"Creating Employee record for existing user {username}")
            ...
            if not oidc_extended_configuration.default_company:
                ...
                try:
                    ...
                    employee.flags.ignore_permissions = True
                    employee.insert()
                    frappe.logger().info(f"Employee {employee.name} created successfully for existing user {username}")
